...
| **hush-gauge** | v1 is mid-flight: M1 (probe panel + detection, G1), M2 (silent leak + causal ablation, G2/G3), and the detachable M3 (off-switch unification, G4) are all ahead. The M1 start-of-stage brief was approved and merged 2026-08-01 (hush-gauge PR #5), freezing D15–D24 before any M1 code runs; the M1 build session is the repo's next action. The card follows whatever each gate decides. |
