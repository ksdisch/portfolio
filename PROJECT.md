...
## Purpose
Public-facing portfolio that presents seven reproduce-and-measure AI-failure projects as one body of work — a method-first index, an honesty charter, and per-project cards — sanitized so a hiring manager can open, run, and poke any linked repo.
...
- The seven project repos evolve in their own repos; this folder only reflects them
...
## Current status
Active — built 2026-07-21; refreshed 2026-07-28. Six of seven linked repos are public; `blind-cite` is parked at M0 and private pending its M1 decision. The portfolio is complete as a shareable artifact; the open work is keeping cards in sync as the underlying projects advance.
...
3. Card `mute-map`'s S1 (7B) and S2 (lexical vs semantic scope) stretches if either is run.
