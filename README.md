...
- **`ghost-patch`, `dim-stage`, and `hush-gauge` headline nulls** — pre-registered, reported as
  headlines, not buried. `hush-gauge` ends on four of them plus a `NOT-RUN`, with **no bar re-tuned
...
- **`hush-gauge` was carded mid-flight and is now carded complete** — it was the first in-progress
  ...
  pre-committed result the card reported whatever it said. The arc closed on four nulls and a
